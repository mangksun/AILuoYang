...
const attractions = [
  ...
  },
  {
    ...
    traffic: JSON.stringify({
      summary: '白云山距洛阳市区约120公里，建议自驾前往，也可乘坐旅游专线。',
      busRoutes: ['洛阳-嵩县旅游专线'],
      metroStations: [],
      parking: '景区入口设有大型停车场',
    }),
...
const foods = [
  ...
  },
  {
    ...
    name: '高记不翻汤',
    category: 'soup',
    description: '不翻汤是洛阳传统名吃，已有120余年历史。其特色是汤面上的薄饼"不翻"，配以海带、粉丝、豆腐皮、木耳等配料，酸辣可口，是洛阳人喜爱的传统小吃。',
...
async function main() {
  ...
  for (const a of attractions) {
    await prisma.attraction.create({ data: a });
  }
  ...
  for (const h of heritages) {
    await prisma.heritage.create({ data: h });
  }
